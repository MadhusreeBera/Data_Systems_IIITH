...
void TableCatalogue::renameTable(string fromName, string toName)
{
    logger.log("TableCatalogue::renameTable");
    Table *table = this->tables[fromName];
    this->tables.erase(fromName);
    this->tables[toName] = table;
}
Table *TableCatalogue::getTable(string tableName)
{
    logger.log("TableCatalogue::getTable " + tableName);
