...
{
    logger.log("BufferManager::getPage " + tableName + " "  + to_string(pageIndex));
...
{
    logger.log("BufferManager::inPool " + pageName + " ?" );
...
Page BufferManager::insertIntoPool(string tableName, int pageIndex)
{
    ...
    {
        pages.pop_front();
        logger.log("insertIntoPool::Page pop_front");
    }
...
{
    logger.log("BufferManager::insertIntoPoolMatrixPage");
...
void BufferManager::removeFromPool(string matrixName, int pageIndex)
{
    int index = -1, i = 0;
    ...
    for (auto page : this->pages)
    {
        logger.log(page.pageName + "in pages");
        if (pageName == page.pageName)
        {
            index = i;
            break;
    ...
    if (index > -1 && index < this->pages.size())
    {
        this->pages.erase(this->pages.begin() + index);
        logger.log("Page removed from pool " + pageName);
    }
    logger.log("exiting page remove   ... ");
}
/**
...
void BufferManager::renameFile(string oldtableName, string newtablename, int pageIndex)
{
    logger.log("BufferManager::renameFile");
    string fileName1 = "../data/temp/"+oldtableName + "_Page" + to_string(pageIndex);
    string fileName2 = "../data/temp/"+newtablename + "_Page" + to_string(pageIndex);
    int value = rename(fileName1.c_str(), fileName2.c_str());

    if (!value) {
        logger.log("MATRIX: RENAME SUCCESSFUL");
    }
    else {
        perror("Error");
        logger.log("MATRIX: RENAME ERROR");
    }
    return;
}

void BufferManager::flushPool()
{
    logger.log("BufferManager::flushPool");
    this->pages.clear();
}
