...
TreePtr LeafNode::insert_key(const Key &key, const RecordPtr &record_ptr)
{
    ...
    // inserting to leafNode
    // cout << "size " << this->size << endl;
    if (data_pointers.size() < FANOUT)
    {
        // cout << "Leaf is not full" << endl;
        // cout << "Key " << key << endl;
        // cout << "Rec ptr " << record_ptr << endl;

        data_pointers.insert({key, record_ptr});
        (this->size)++;
        this->dump();
        return NULL_PTR;
    }
    else
    {
        ...
        LeafNode *splitLeafNode = new LeafNode();
        new_leaf = splitLeafNode->tree_ptr;
        splitLeafNode->next_leaf_ptr = this->next_leaf_ptr;


        //keeping ceil(f/2) in first leaf and storing rest in split node
        while (data_pointers.size() > MIN_OCCUPANCY)
        {
            Key k = data_pointers.rbegin()->first;
            RecordPtr r = data_pointers.rbegin()->second;
            splitLeafNode->data_pointers.insert({k, r});
            splitLeafNode->size++;
            data_pointers.erase(prev(data_pointers.end()));
        }
